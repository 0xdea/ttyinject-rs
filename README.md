...
> [!TIP]
> As an alternative to pre-built release binaries, you can build your own binary via [crates.io](https://crates.io/crates/ttyinject-rs) with `cargo install ttyinject-rs --locked`, use the library in your own projects with `cargo add ttyinject-rs`, or compile it from the source repository. For troubleshooting, you can run the binary in verbose mode by providing any argument to it.
